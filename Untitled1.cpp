...
struct PatientNode {
    ...
    PatientNode* prev;    // Pointer to the previous patient in queue
    PatientNode* next;    // Pointer to the next patient in queue
...
public:
    ...
    void InsertAtPosition(int patientId, int position) {
        ...
        if (current->next != nullptr) {
            ...
            back = newPatient; // Update back if inserting at the end
        }
    ...
    /**
     * Display the queue from front to back
     */
    void displayQueue() {
        if (front == nullptr) {
    ...
    /**
     * Display the queue from back to front
     */
    ...
    /**
     * Display current queue status information
     */
    void displayStatus() {
...
/**
 * Main function to demonstrate the ERQueue functionality
 */
int main() {
    ...
    emergencyRoom.DeleteFromBegining();              // One treated
    
    // Add another normal patient
    emergencyRoom.InsertAtEnd(300);       // Another patient shows up
